...
def get_text(letters: str, words_count: int = 20) -> str:
    ...
    filtered_words = [word for word in words if set(word).issubset(set(letters))]
...
def result_screen(
    ...
    stdscr.clear()
    stdscr.addstr(f"TouchType | {modes[mode_name]['name']}\n", curses.color_pair(1))
    stdscr.addstr("\nResult\n", curses.color_pair(2))
    stdscr.addstr("------\n", curses.color_pair(2))

    stdscr.addstr(f"{time_elapsed} minutes ({round(time_elapsed*60)} sec) elapsed\n")
...
def start(mode: str, stdscr: curses.window):
    ...
    while True:
        stdscr.clear()
        stdscr.addstr(0, 0, f"TouchType | {modes[mode]['name']}", curses.color_pair(1))
...
def menu_screen(stdscr: curses.window):
    ...
    if (not key.isdigit()) or (int(key) not in range(0, 6)):
        menu_screen(stdscr)
    elif key == "0":
        exit()  # exit program
...
def check_terminal_size(stdscr):
    ...
    if height < 20 or width < 80:
        raise curses.error("Terminal window too small. Minimum 80x20 required.")
